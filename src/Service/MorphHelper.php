...
final class MorphHelper
{
    ...
    public function candidates(string $lang, string $token): array
    {
        ...
        if ($l === 'en' || $l === 'eng') {
            ...
            foreach ((array) $singToken as $sing) { $add((string) $sing); }
            foreach ((array) $singLower as $sing) { $add((string) $sing); }

            // Plurals are rarely needed for source lookup, but harmless to include.
            foreach ((array) $plurToken as $pl) { $add((string) $pl); }
            foreach ((array) $plurLower as $pl) { $add((string) $pl); }
        }
